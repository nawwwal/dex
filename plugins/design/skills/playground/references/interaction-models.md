...
canvas/node map -> relationships and spatial thinking -> draggable nodes, edges, zoom, filters, click-to-comment, inspector -> annotated map export with selected nodes, comments, and unresolved questions -> avoid graph soup with every node at equal weight
...
simulator -> cause and effect under changing conditions -> scenario switches, coupled variables, repeated runs, derived metrics, warnings, recommended ranges -> recommended tuning prompt with evidence and assumptions -> avoid fake dashboards with numbers that do not derive from state
...
artifact editor -> concrete generated object or surface -> direct manipulation, live preview, undo/redo, presets, import/export config -> implementation prompt or config JSON -> avoid decoration-only editing where controls do not affect the artifact's meaning
