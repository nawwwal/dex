#!/usr/bin/env python3
"""Validate the durable playground eval suite shape and coverage."""
...
def main() -> None:
    ...
    if data["skill_name"] != "playground":
        fail("skill_name must be playground")
    ...
    if not isinstance(evals, list) or len(evals) < 35:
        fail("expected at least 35 eval cases for exhaustive playground coverage")
    ...
    missing_migrated = MIGRATED_V1_IDS - seen_ids
    if missing_migrated:
        fail(f"missing migrated v1 eval ids: {sorted(missing_migrated)}")

    if trigger_values[True] < 28:
        fail("expected at least 28 positive trigger cases")
    if trigger_values[False] < 5:
        fail("expected at least 5 negative-control cases")

    visual_cases = [case for case in evals if case["category"] == "visual-quality"]
    if len(visual_cases) < 5:
        fail("expected at least five visual-quality cases")

    html_cases = [case for case in evals if case["category"] == "html-contract"]
    if len(html_cases) < 5:
        fail("expected at least five html-contract cases")

    explore_cases = [case for case in evals if case["category"] == "explore-mode"]
    if len(explore_cases) < 4:
        fail("expected at least four explore-mode cases")

    companion_cases = [case for case in evals if case["category"] == "companion-routing"]
    if len(companion_cases) < 5:
        fail("expected at least five companion-routing cases")
