#!/usr/bin/env python3
"""Validate the durable crux eval suite shape and coverage."""
...
def main() -> None:
    suite_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).parents[1] / "evals" / "exhaustive-suite.json"
    ...
    if data["skill_name"] != "crux":
        fail("skill_name must be crux")
    ...
    if not isinstance(evals, list) or len(evals) < 24:
        fail("expected at least 24 eval cases for exhaustive crux coverage")
    ...
    if trigger_values[True] < 18:
        fail("expected at least 18 positive trigger cases")
    if trigger_values[False] < 3:
        fail("expected at least 3 negative-control cases")
