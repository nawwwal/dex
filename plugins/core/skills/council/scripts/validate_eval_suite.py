#!/usr/bin/env python3
"""Validate the durable council eval suite shape and coverage."""
...
def main() -> None:
    ...
    if data["skill_name"] != "council":
        fail("skill_name must be council")
    ...
    if not isinstance(evals, list) or len(evals) < 40:
        fail("expected at least 40 eval cases for exhaustive council coverage")
    ...
    if trigger_values[True] < 30:
        fail("expected at least 30 positive trigger cases")
    ...
    domain_design = [case for case in evals if case["category"] == "domain-design"]
    if len(domain_design) < 3:
        fail("expected at least three domain-design cases")
