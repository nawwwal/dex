...
| Sessions today = 0 AND day = Monday | HIGH | Morning Mode + /assistant plan |
| Sessions today = 0 AND day = Friday | HIGH | Morning Mode + week kickoff reminder |
| Time after 6pm IST OR "wrapping up" / "done for today" | HIGH | /assistant eod |
| Day = Friday + "wrapping up" | HIGH | /assistant eod + /assistant week |
...
| "plan my week" / "7 days" / "7plan" | /assistant plan |
| "tell [name]" / "message [name]" / "dm [name]" | Communication Mode |
| "trace [X]" / "history of [X]" | /assistant trace on X |
| "ideas for [X]" | /assistant leverage on X |
| "map" | /assistant graph |
| "answer as me" / "ghost write" | Ghost Mode |
| "emerge" / "any patterns" | /assistant emerge |
| "drift" / "what's quiet" | /assistant drift |
| "backlinks" | /assistant graph |
...
| Signal | What to run |
|---|---|
| Google Doc / Sheets URL in message | Read doc + /assistant emerge for connections to current projects |
| Project name mentioned + question | Context Mode (QMD + decisions + sessions for that project) |
| "stuck" / "don't know" + topic | /assistant challenge + /assistant leverage on that topic |
| Frustration signal ("annoyed", "ugh", frustrated tone) | Acknowledge + /drift (what's piling up?) |
| Strategy doc discussed | /assistant emerge (what connects to current work?) + career implications |
...
| ANY project 7+ days silent | /assistant drift is MANDATORY — surface in every mode |
...
2. **/assistant drift flag** always included if silences exist (can be a one-liner, doesn't need to be the focus)
