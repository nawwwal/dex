...
1. **Prompt receipt**: inject the already-resolved default vault, qmd-first retrieval rule, no Tolaria search rule, and writeback audit reminder at `UserPromptSubmit`.
...
- The emitted receipt names the configured vault, says not to call `list_vaults` just to confirm it, names the qmd collection, qmd source-read rule, Tolaria writeback role, and "no update needed" audit.
