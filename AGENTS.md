...
The release skill bumps the selected plugin version across the plugin's Claude and Codex manifests plus the Claude marketplace metadata, then commits, tags, and pushes. Must be on `main` with a clean worktree. Skill edits/removals are patch releases; major is reserved for plugin or marketplace contract changes.
